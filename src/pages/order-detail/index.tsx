...
const OrderDetailPage: React.FC = () => {
  ...
  const steps = [
    ...
    { label: '门店确认', time: order.confirmedAt || '--', desc: '等待门店确认', done: order.status !== 'pending' },
    {
      label: '宠物入住',
      time: order.checkInDate,
      desc: '入住当天办理入住',
      done: order.status === 'ongoing' || order.status === 'completed'
    },
    {
      label: '服务完成',
      ...
      <ScrollView scrollY>
        <View className={styles.container}>
          ...
          <View className={styles.card}>
            <View className={styles.cardTitle}>
              <Text>🐾</Text>
              <Text>寄养宠物 ({order.pets.length}只)</Text>
            </View>
            {order.pets.map((pet) => (
          ...
          <View className={styles.card}>
            ...
            <View className={styles.depositRow}>
              <View className={styles.depositLeft}>
                ...
                <Text className={styles.depositText}>押金 ¥{order.deposit.toFixed(2)}</Text>
              </View>
          ...
          <View className={styles.card}>
            ...
            <View className={styles.orderNoRow}>
              ...
              <Text
                className={styles.orderNoValue} onClick={() => handleCopy(order.orderNo)}>
                {order.orderNo}
                <Text>📋</Text>
              </Text>
            </View>
            ...
            {order.remark && (
              <View className={styles.orderNoRow}>
                <Text className={styles.orderNoLabel}>订单备注</Text>
